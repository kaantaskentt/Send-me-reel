...
async function localEndpoints(web) {
  ...
  assert(endpoints.some(({ pathname }) => pathname === "/replicate/local"), "Local studio page missing from smoke checks");
  assert(endpoints.some(({ pathname }) => pathname.startsWith("/api/local/")), "Local API checks are empty");
  return endpoints;
}
