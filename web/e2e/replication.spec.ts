...
test("workbench prepares from the API, pairs locally and confirms browser actions without claiming success", async ({ page }) => {
  ...
  await page.route("http://127.0.0.1:43187/**", async (route) => {
    ...
    if (path === "/health") return route.fulfill({ headers, json: { status: "ready", version: 1, platform: "darwin", capabilities: { terminal: false, harnesses: { codex: false, claude: false }, browser: { configured: true } } } });
  ...
  await expect(session.getByRole("button", { name: "Yes, do this" })).toBeInViewport();
  ...
  await expect(page.getByRole("img", { name: "Expanded browser screenshot" })).toBeVisible();
  ...
  await page.getByRole("button", { name: "Yes, do this" }).click();
  await expect(page.getByText("Finished · check the result", { exact: true })).toBeVisible();
...
test("development rehearsal replays approvals, manual handoff and restart without live requests", async ({ page }) => {
  const liveRequests: string[] = [];
  const errors: string[] = [];
  page.on("pageerror", (error) => errors.push(error.message));
  page.on("request", (request) => {
    const url = new URL(request.url());
    if (url.pathname.startsWith("/api/") || url.port === "43187" || url.hostname.endsWith("openai.com")) liveRequests.push(request.url());
  });
  await page.goto("/replicate/demo");
  await page.getByRole("button", { name: "Start rehearsal", exact: true }).click();
  const session = page.getByRole("dialog", { name: "Browser walkthrough", exact: true });
  await expect(session).toBeVisible();
  await expect(session.getByText("Rehearsal · No live actions", { exact: false })).toBeVisible();
  await session.getByRole("button", { name: "Toggle fullscreen presentation" }).click();
  await expect.poll(() => page.evaluate(() => !!document.fullscreenElement)).toBe(true);
  await session.getByRole("button", { name: "Toggle fullscreen presentation" }).click();
  await expect.poll(() => page.evaluate(() => !!document.fullscreenElement)).toBe(false);
  const capture = session.getByRole("img", { name: "Current browser view for this task" });
  await expect(capture).toHaveAttribute("src", "/rehearsal/light-desktop.png");
  await expect(session.getByRole("button", { name: "Yes, do this" })).toBeInViewport({ ratio: 1 });
  await page.screenshot({ path: `${screenshots}/rehearsal-desktop-start.png` });
  await session.getByRole("button", { name: "Yes, do this" }).click();
  await expect(capture).toHaveAttribute("src", "/rehearsal/dark-desktop.png");
  await expect(session.getByText("Open the mobile preview to inspect how the layout adapts.")).toBeVisible();
  await page.screenshot({ path: `${screenshots}/rehearsal-desktop-changed.png` });
  await session.getByRole("button", { name: "Yes, do this" }).click();
  await expect(capture).toHaveAttribute("src", "/rehearsal/dark-mobile.png");
  await expect(session.getByRole("button", { name: "Continue rehearsal" })).toBeVisible();
  await session.getByRole("button", { name: "Continue rehearsal" }).click();
  await expect(session.getByText("Rehearsal complete", { exact: true })).toBeVisible();
  await page.screenshot({ path: `${screenshots}/rehearsal-desktop-finished.png` });
  await session.getByRole("button", { name: "Restart", exact: true }).click();
  await expect(capture).toHaveAttribute("src", "/rehearsal/light-desktop.png");
  await session.getByRole("button", { name: "Decline", exact: true }).click();
  await expect(session.getByText(/Action rejected. The example stayed unchanged/)).toBeVisible();
  await expect(capture).toHaveAttribute("src", "/rehearsal/light-desktop.png");
  await session.getByRole("button", { name: "Restart", exact: true }).click();
  await page.setViewportSize({ width: 390, height: 844 });
  await expect(session.getByRole("button", { name: "Yes, do this" })).toBeInViewport({ ratio: 1 });
  expect(await session.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true);
  await page.screenshot({ path: `${screenshots}/rehearsal-mobile.png` });
  await session.getByRole("button", { name: "Stop task", exact: true }).click();
  await expect(session.getByText("Stopped", { exact: true })).toBeVisible();
