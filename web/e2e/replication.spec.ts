...
test("development rehearsal replays approvals, manual handoff and restart without live requests", async ({ page }) => {
  ...
  const session = page.getByRole("dialog", { name: "Browser walkthrough", exact: true });
  await expect(session).toBeVisible();
  await expect(session.getByText("Rehearsal · test example", { exact: true })).toBeVisible();
  await session.getByRole("button", { name: "Toggle fullscreen presentation" }).click();
  await expect.poll(() => page.evaluate(() => !!document.fullscreenElement)).toBe(true);
  await session.getByRole("button", { name: "Toggle fullscreen presentation" }).click();
  await expect.poll(() => page.evaluate(() => !!document.fullscreenElement)).toBe(false);
  const capture = session.getByRole("img", { name: "Current local browser view with proposed action targets" });
  await expect(capture).toHaveAttribute("src", "/rehearsal/light-desktop.png");
  await expect(session.getByRole("button", { name: "Approve this action" })).toBeInViewport({ ratio: 1 });
  await page.screenshot({ path: `${screenshots}/rehearsal-desktop-start.png` });
  await session.getByRole("button", { name: "Approve this action" }).click();
  ...
  await expect(session.getByText("Rehearsal complete · no live task executed", { exact: true })).toBeVisible();
  ...
  await expect(capture).toHaveAttribute("src", "/rehearsal/light-desktop.png");
  await session.getByRole("button", { name: "Reject", exact: true }).click();
  ...
  await session.getByRole("button", { name: "Stop rehearsal", exact: true }).click();
  await expect(session.getByText("Session stopped", { exact: true })).toBeVisible();
