...
export default function BrowserSession({ run, dialogRef, actionBusy, error, sourceTitle, planTitle, onControl, rehearsal }: Props) {
  ...
  return <dialog ref={dialogRef} aria-labelledby="browser-session-title" className="m-auto max-h-[94dvh] w-[1160px] max-w-[calc(100vw-24px)] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-950 p-0 text-white shadow-2xl backdrop:bg-slate-950/75 backdrop:backdrop-blur-sm">
    ...
    <div className="grid min-w-0 gap-5 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <section aria-label="Next browser action" className="min-w-0 lg:sticky lg:top-24 lg:self-start">
        ...
        <p className="mt-4 text-[10px] leading-relaxed text-slate-500">{rehearsal ? "This rehearsal replays captures from a local test page. Your approvals change the example only. Use Restart to present it again." : "Back to plan keeps the browser session open. Stop ends it. Browser guidance sends captures and visible text to OpenAI using your local API key."}</p>
      </section>
