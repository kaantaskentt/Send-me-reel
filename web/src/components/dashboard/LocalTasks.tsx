...
export default function LocalTasks({ token }: { token?: string }) {
  ...
  return <>
    ...
    {open && !selected && <StudioDialog title="Your tasks" onClose={close}>
      ...
      <div className={styles.list}>{runs.map(run => <button type="button" key={run.id} className={styles.listItem} onClick={() => { setSelected(run); setOutput(null); setTab("activity"); }}>{run.executor === "browser" ? <Monitor size={18} /> : <Terminal size={18} />}<div><strong>{run.title || run.goal || "Local task"}</strong><small>{runStatusLabel(run.status)} · {new Date(run.createdAt).toLocaleString()}</small></div><ArrowUpRight size={15} /></button>)}</div>
