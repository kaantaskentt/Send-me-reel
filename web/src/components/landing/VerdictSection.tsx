...
/*
 * VerdictSection — clean 4-step workflow, centered, responsive
 */
    ...
    <section id="how-it-works" className="py-20 lg:py-28" style={{ background: "white" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 clamp(1rem, 4vw, 4rem)" }}>
        ...
        <div ref={stepsRef} className="stagger-children">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 0, position: "relative", marginBottom: "2.5rem" }} className="hidden sm:grid">
            ...
            {STEPS.map((step, i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", position: "relative", zIndex: 1 }}>
                <div style={{
                  ...
                  boxShadow: i === 3 ? "0 4px 20px rgba(249,115,22,0.3)" : "0 2px 8px rgba(0,0,0,0.05)",
                }}>
                  {step.icon}
                </div>
                <span style={{ fontSize: 10, fontWeight: 700, color: "#f97316", background: "#fff7ed", border: "1px solid #fed7aa", padding: "1px 8px", borderRadius: 20, fontFamily: "'JetBrains Mono', monospace", marginBottom: 8 }}>
                  {i + 1}
                </span>
          ...
          <div className="sm:hidden" style={{ display: "flex", flexDirection: "column", gap: 0, position: "relative", marginBottom: "2rem" }}>
            {/* Vertical line */}
