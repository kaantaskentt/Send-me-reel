...
export interface ContentReply { answer: string; suggestions: string[]; actions: ContentAction[]; evidence: number[]; allowedUrls?: string[] }
...
export const quickTakePrompt = "Give me a quick take in 2 or 3 short sentences, under 90 words. Name the useful ideas or tools this AI content shows. Then give three short, specific suggested questions I can click: for example find the referenced sites, explain a skill, or adapt an idea for me. Focus on helping me use the content, not analyzing the creator's marketing. No actions, no external search, and no plan yet. Cite relevant observation indexes only.";
...
export function evidenceContext(analysis: Analysis) {
  const frames = (analysis.frame_descriptions ?? []).slice(0, 2000).map((frame, index) => ({ ...(typeof frame === "object" && frame ? frame : { description: String(frame) }), index }));
  ...
  return {
    sourceUrl: analysis.source_url, title: analysis.metadata?.title, durationSeconds: analysis.metadata?.duration,
    transcript: transcript.slice(0, 100_000), transcriptTruncated: transcript.length > 100_000,
    observations: selected, totalObservations: frames.length, observationIndexSampled: selected.length < frames.length,
    captureCoverage: (() => { const evidence = analysis.metadata?.source_evidence as { capture_complete?: boolean; warnings?: string[]; text?: unknown; native_video?: { coverage?: unknown } } | undefined; return { complete: evidence?.capture_complete, warnings: evidence?.warnings, text: evidence?.text, nativeCoverage: evidence?.native_video?.coverage }; })(),
    caption: (analysis.caption ?? "").slice(0, 60_000), captionTruncated: (analysis.caption?.length ?? 0) > 60_000,
  };
}

export const contentReplySchema = {
  type: "object", additionalProperties: false, required: ["answer", "suggestions", "actions", "evidence"],
  properties: {
...
export function parseContentReply(raw: string, frameCount: number, allowedUrls: Set<string>): ContentReply {
  ...
  return { answer, actions, allowedUrls: [...allowedUrls], suggestions: value.suggestions.filter((s: unknown): s is string => typeof s === "string" && s.length > 0 && s.length <= 240).slice(0, 3), evidence: [...new Set<number>(value.evidence.filter((n: unknown): n is number => Number.isInteger(n) && Number(n) >= 0 && Number(n) < frameCount))].slice(0, 8) };
}
