...
export function parseReplicationDraft(content: string, { id, goal, mode, sourceUrl, evidence, warnings }: PlanContext & { id: string }) {
      ...
      if (Array.isArray(draft.steps) && draft.steps.some((step: { kind?: unknown; evidenceIds?: unknown }) => step?.kind === "observed" && Array.isArray(step.evidenceIds) && step.evidenceIds.length > 0 && step.evidenceIds.every((ref: unknown) => typeof ref === "string" && ref.startsWith("user-context-")))) throw new Error("User preferences cannot prove an observed source step");
