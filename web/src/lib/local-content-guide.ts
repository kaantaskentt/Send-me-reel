...
Write a specific title of at most 10 words about the main idea in everyday language, like Make a website from a picture. Avoid a list of proper names or jargon such as routing and configurations. Write one sentence (under 22 words) saying what the content shows. Do not confuse a creator's claim with a proven result. If evidence is thin, say so briefly. Never imply every frame was inspected. A project, window or tab name is not necessarily a tool: do not use it as the user's project or offer to install it without clear evidence of its identity. A project name visible in the creator's app is their example; offer a small test in a new project instead. Do not promise to recover a complete list or a hidden resource from a creator's count alone; say Find the names shown.
