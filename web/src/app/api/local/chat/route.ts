...
export async function POST(request: NextRequest) {
  ...
  active = true;
  try {
    let body: { analysisId: string; message?: string; brief?: boolean; guide?: boolean };
    ...
    if (!body || Object.keys(body).some(k => !["analysisId", "message", "brief", "guide"].includes(k)) || typeof body.analysisId !== "string" || (body.brief !== undefined && typeof body.brief !== "boolean") || (body.guide !== undefined && typeof body.guide !== "boolean") || (body.guide && (body.brief || body.message !== undefined)) || (!body.brief && !body.guide && (typeof body.message !== "string" || !body.message.trim() || body.message.length > 4000))) return NextResponse.json({ error: "Write a question of up to 4,000 characters." }, { status: 400 });
    const analysis = await readLocalAnalysis();
    ...
    if (body.guide && conversation.guide) {
      try { conversation.guide = parseContentGuide(conversation.guide, analysis); return NextResponse.json(conversation, { headers: { "Cache-Control": "no-store" } }); }
      catch { /* Old or incomplete choices can be rebuilt without changing chat history. */ }
    }
    ...
    if (body.guide) {
      const result = await createContentGuide(analysis);
      conversation.guide = result.guide;
      await writeConversation(conversation);
      await fs.appendFile(path.join(localStudioRoot, "chat-usage.jsonl"), JSON.stringify({ at: new Date().toISOString(), analysisId: analysis.id, purpose: "choices", ...result.usage }) + "\n", { mode: 0o600 });
