...
import { sourceImageMime } from "@/lib/source-frames";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ index: string }> }) {
  ...
  return new NextResponse(bytes, { headers: { "Content-Type": sourceImageMime(filename)!, "Cache-Control": "no-store", "X-Content-Type-Options": "nosniff" } });
}
